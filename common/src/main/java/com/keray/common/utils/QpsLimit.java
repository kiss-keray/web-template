...
public final class QpsLimit {
    private static final ConcurrentMap<String, SlidingWindow> WINDOWS = new ConcurrentHashMap<>(8);
    public static final int DEFAULT_WAIT_COUNT = 10;
    public static final int DEFAULT_WAIT_WINDOW_WIDTH_SECONDS = 60;
     ...
     */
    public static void acceptLimit(String sign, long qps, Strategy strategy, Integer wait) throws QPSFailException {
        acceptLimit(sign, qps, strategy, wait, DEFAULT_WAIT_WINDOW_WIDTH_SECONDS);
    }
     ...
     */
    public static void acceptLimit(String sign, long qps, Strategy strategy, Integer wait, int seconds) throws QPSFailException {
        wait = wait == null ? DEFAULT_WAIT_COUNT : wait;
        if (checkQps(sign, qps, seconds)) {
            switch (strategy) {
                case throw_exception:
                    throw new QPSFailException("qps limit throw exception");
                case wait:
                    wait(wait);
                    break;
                case noting_log:
                    log.warn("{} :qps to achieve limit . limit {}", Thread.currentThread().getName(), qps);
                    break;
                default:
                    break;
            }
    ...
    public static <R> R acceptLimit(String sign, long qps, Supplier<R> pass, Supplier<R> refused) {
        return acceptLimit(sign, qps, DEFAULT_WAIT_WINDOW_WIDTH_SECONDS, pass, refused);
    }

    public static <R> R acceptLimit(String sign, long qps, int seconds, Supplier<R> pass, Supplier<R> refused) {
        if (checkQps(sign, qps, seconds)) {
            return pass.get();
        }
        return refused.get();
    ...
    public static <R> R acceptLimit(String sign, long qps, Supplier<R> refused) {
        return acceptLimit(sign, qps, DEFAULT_WAIT_WINDOW_WIDTH_SECONDS, refused);
    }

    public static <R> R acceptLimit(String sign, long qps, int seconds, Supplier<R> refused) {
        if (checkQps(sign, qps, seconds)) {
            return null;
        }
        return refused.get();
     ...
     * 检查调用qps是否超过限制
     */
    private static boolean checkQps(String sign, long qps, int seconds) {
        if (!WINDOWS.containsKey(sign)) {
            synchronized (QpsLimit.class) {
                if (!WINDOWS.containsKey(sign)) {
                    WINDOWS.put(sign, new SlidingWindow(sign, qps, seconds));
                }
            }
        }
    ...
    private static class SlidingWindow {
        ...
        //qps取样4段 15*4 60秒
        private final int ALL_PIECEWISE;
        private volatile long start = 0;
        private volatile long end = 0;
        //一分钟分四段 每15秒设置一段
        private volatile int[] slidingRecord;
        private final ReentrantReadWriteLock readWriteLock = new ReentrantReadWriteLock();
        private final Object lastWindowCLock = new Object();

        private SlidingWindow(String sign, long limitQps, int seconds) {
            this.sign = sign;
            this.limitQps = limitQps;
            ALL_PIECEWISE = seconds / PIECEWISE;
            slidingRecord = new int[ALL_PIECEWISE];
            init();
        }
        ...
        private void init() {
            ...
            start = nowSeconds() - PIECEWISE * (ALL_PIECEWISE - 1);
            end = nowSeconds();
        }
    ...
    public static class QPSFailException extends RuntimeException {
        ...
        public QPSFailException(String message, Throwable cause) {
            super(message, cause);
        }

        public QPSFailException(Throwable cause) {
            super(cause);
        }

        public QPSFailException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace) {
